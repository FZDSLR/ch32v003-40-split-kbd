...
uint8_t RB_Write_Byte(ring_buffer *rb_handle, uint8_t data)
{
    ...
    if(rb_handle->tail > (rb_handle->max_Length - 1)){rb_handle->tail = 0;}
 ...
 * \param[in] read_and_clean: 擦除标志
 * \return 返回缓冲区头部读指定长度字节的结果
 *      \arg RING_BUFFER_SUCCESS: 读取成功
 *      \arg RING_BUFFER_ERROR: 读取失败
*/
uint8_t RB_Read_String(ring_buffer *rb_handle, uint8_t *output_addr, uint32_t read_Length, uint8_t read_and_clean)
{
